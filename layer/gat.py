...
class GATConv(MessagePassing):
    def __init__(self, in_channels, out_channels, heads, dropout, negative_slope = 0.2, **kwargs):
        ...
        self.edge_lin = torch.nn.Linear(out_channels*2, out_channels*2)
        self.att_l = Parameter(torch.Tensor(1, heads, out_channels))
        self.att_r = Parameter(torch.Tensor(1, heads, out_channels))

    def forward(self, x, edge_index, size=None):
        H, C = self.heads, self.out_channels
        x_l = x_r = self.lin_l(x).view(-1, H, C)  # shape: [num_nodes, *]
        alpha_l = (x_l * self.att_l).sum(dim=-1)  # shape: [num_nodes, *]
        alpha_r = (x_r * self.att_r).sum(dim=-1)  # shape: [num_nodes, *]
        out = self.propagate(edge_index, x=(x_l, x_r), alpha=(alpha_l, alpha_r), size=size)
        out = self.edge_lin(out.reshape((-1, self.out_channels * 2)))
        return out
    ...
    def __init__(self, input_dim, args, num_nodes, num_classes):
        ...
        for i in range(num_layers-1):
            conv = torch_geometric.nn.Sequential('x, edge_index',
                [(GATConv(gat_input_dim, hidden_dim, heads = num_heads, dropout=dropout),'x, edge_index-> x'),
        ...
        if self.pooling == "concat":
            ...
            """
            conv = torch_geometric.nn.Sequential('x, edge_index, edge_attr', [
                (MPGATConv(hidden_dim * num_nodes, hidden_dim, heads=num_heads, dropout=dropout,
                                 gat_mp_type=gat_mp_type),'x, edge_index, edge_attr -> x'),
                nn.Linear(hidden_dim*num_heads, 64),
                nn.LeakyReLU(negative_slope=0.2),
                nn.Linear(64, node_dim),
